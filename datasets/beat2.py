...
class BEAT2DatasetEamgeFootContact(BEAT2Dataset):
    ...
    def __getitem__(self, item):
        ...
        motion_tensor = torch.from_numpy(motion).float()  #[64, 165]
        audio_tensor = torch.from_numpy(audio).float() #[34112]
        expressions_tesnor = torch.from_numpy(expressions).float()  #[64, 100]
        trans_tensor = torch.from_numpy(trans).float() #[64, 3]
        foot_contact_tensor = torch.from_numpy(foot_contact).float() #[64, 4]
