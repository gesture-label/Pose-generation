...
def inference_fn(cfg, model, device, test_path, save_path, **kwargs):
    ...
    for test_file in tqdm(test_list, desc="Testing"):
        ...
        poses = torch.from_numpy(motion_data["poses"]).unsqueeze(0).to(device).float()  #[1, 1703, 165]
        ...
        bs, t, _ = poses.shape
        poses_6d = rc.axis_angle_to_rotation_6d(poses.reshape(bs, t, -1, 3)).reshape(bs, t, -1)  #[1, 1703, 330]
        ...
        motion_pred = motion_all["motion_axis_angle"]  #[1, 1710, 165]
        t = motion_pred.shape[1]
        motion_pred = motion_pred.cpu().numpy().reshape(t, -1)
        expression_pred = motion_all["expression"].cpu().numpy().reshape(t, -1)  #(1710, 100)
...
def train_val_fn(cfg, batch, model, device, mode="train", **kwargs):
    ...
    motion_vq = kwargs["motion_vq"]
    motion_gt = batch["motion"].to(device)  #[56, 64, 330])
    audio = batch["audio"].to(device)
    expressions_gt = batch["expressions"].to(device)  #[56, 64, 100]
    ...
    if mode == "train":
        if cfg.solver.max_grad_norm > 0:   #梯度裁剪
...
def init_env():
    ...
    parser.add_argument("--config", type=str, default="./configs/emage_audio.yaml")
