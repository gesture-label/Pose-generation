...
class EmageVQVAEConv(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        return super().from_pretrained(pretrained_model_name_or_path, local_files_only=True, *args, **kwargs)
        # 否则走默认 Hugging Face Hub 逻辑
...
class EmageVQModel(nn.Module):
    ...
    def decode(self, face_index=None, upper_index=None, hands_index=None, lower_index=None, 
        ...
        if face_index is not None:
            ...
            face_jaw_6d, expression = face_mix[:, :, :6], face_mix[:, :, 6:]  #[1, 64, 6],[1, 64, 100]
            face_jaw = rotation_6d_to_axis_angle(face_jaw_6d)  #[1, 64, 3]
        else:
        ...
        if upper_index is not None:
            ...
            upper = rotation_6d_to_axis_angle(upper_6d.reshape(bs, t, -1, 6)).reshape(bs, t, -1)  # bs, t, 39
        ...
        if hands_index is not None:
            hands_6d = self.vq_model_hands.decode(hands_index)  #[1, 64, 180]
            hands = rotation_6d_to_axis_angle(hands_6d.reshape(bs, t, -1, 6)).reshape(bs, t, -1)  #[1, 64, 90]
        ...
        if lower_index is not None:
            ...
            lower_6d, transfoot = lower_mix[:, :, :-7], lower_mix[:, :, -7:]
            lower = rotation_6d_to_axis_angle(lower_6d.reshape(bs, t, -1, 6)).reshape(bs, t, -1)  #[1, 64, 27]
        else:
...
class EmageAudioModel(PreTrainedModel):
    ...
    def forward(self, audio, speaker_id, masked_motion, mask, use_audio=True):
        # mask motion
        masked_embeddings = self.mask_embedding.expand_as(masked_motion)   #为掩码部分生成统一的嵌入表示
        masked_motion = torch.where(mask==1, masked_embeddings, masked_motion)  #[1, 64, 337]

        # motion token (spatial hints)
        body_hint = self.motion_encoder(masked_motion)  #[1, 64, 256]
        body_hint_body = self.bodyhints_body(body_hint)  #[1, 64, 256]
        body_hint_face = self.bodyhints_face(body_hint)  #[1, 64, 256]
        
        audio2face_fea = self.audio_encoder_face(audio)  #[1, 64, 256]
        audio2body_fea = self.audio_encoder_body(audio)  #[1, 64, 256]
        ...
        speaker_motion_fea_proj = self.speaker_embedding_body(speaker_id).repeat(1, t, 1)  #[1, 64, 768]
        speaker_face_fea_proj = self.speaker_embedding_face(speaker_id).repeat(1, t, 1)  #[1, 64, 768]
        
        audio2face_fea_proj = self.audio_face_motion_proj(torch.cat([audio2face_fea, body_hint_face], dim=2))  #[1, 64, 768]
        ...
        face_proj = self.position_embeddings(speaker_face_fea_proj)  #[1, 64, 768]
        decode_face = self.face_motion_decoder(tgt=face_proj.permute(1,0,2), memory=audio2face_fea_proj.permute(1,0,2)).permute(1,0,2)
        face_latent = self.face_out_proj(decode_face)  #[1, 64, 768]
        classify_face = self.face_cls(face_latent)  #[1, 64, 256]
        ...
        motion_fea = self.motion_self_encoder(masked_motion_proj.permute(1,0,2)).permute(1,0,2)  #[1, 64, 768]
        ...
        motion_fea = motion_fea + audio2body_fea_cross #[1, 64, 768]

        # mlp
        upper_latent = self.motion2latent_upper(motion_fea)  #[1, 64, 768]
        hands_latent = self.motion2latent_hands(motion_fea)  #[1, 64, 768]
        lower_latent = self.motion2latent_lower(motion_fea)  #[1, 64, 768]
        ...
        upper_latent = self.motion_out_proj_upper(upper_latent + motion_upper_refine)  #[1, 64, 256]
        ...
        # decode body
        classify_upper = self.motion_cls_upper(upper_latent)  #[1, 64, 256]
    ...
    def inference(self, audio, speaker_id, vq_model, masked_motion=None, mask=None):
        ...
        # Autoregressive inference   分块处理，每64帧为一块
        ...
        last_motion = masked_motion[:, :pre_frames, :]  #[1,4,337]
        for i in range(rounds):
            ...
            _, cls_face =  torch.max(F.log_softmax(net_out_val["cls_face"], dim=2), dim=2)  #[1, 64]
            ...
            face_latent = net_out_val["rec_face"] if self.cfg.lf > 0 and self.cfg.cf == 0 else None  #[1, 64, 256]
